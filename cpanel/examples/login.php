<?php 
  ...
  if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(filter_var($email,FILTER_VALIDATE_EMAIL)){
      $admin = findAdmin($db,$email);
      if($admin){
        session_start();
        if(hash_equals($admin["a_password"],$psd)){
          $_SESSION["a_connected"] = true;
          $_SESSION["a_nom"] = $admin["nom"];
          $_SESSION["a_id"] = $admin["id"];
          $_SESSION["a_username"] = $admin["username"];
          $_SESSION["a_prenom"] = $admin["prenom"];
          $_SESSION["a_pays"] = $admin["pays"];
          $_SESSION["a_ville"] = $admin["ville"];
          $_SESSION["a_description"] = $admin["description"];
          $_SESSION["a_email"] = $admin["email"];
          
          $_SESSION["a_profile"] = $admin["image"];
